...
const PurchaseBillingPage = ({}) => {
  ...
  //     "totalAmount": "1115.00",
  //     "totalQuantity": "185",
  //     "totalAllAmounts": "37405.00",
  //     "date": "2023-09-11T04:03:18.039Z",
  ...
  console.log("transcationFormData", transcationFormData);
  ...
  const saveEvent = () => {
    ...
    // Calculate the next ID based on the length of existing data
    //const nextId = parseInt(existingData.length) + 1;

    // Check if the ID is null and assign the appropriate value
    // const nextId =
    //   transcationFormData.id !== 0
    //     ? parseInt(existingData.length) + 1
    //     : Math.max(...existingData.map((item: any) => item.id), 0) + 1;

    // //Set the ID for the current transaction
    // setTranscationFormData((prevData: any) => ({
    //   ...prevData,
    //   id: nextId,
    // }));
  ...
  return (
    <>
      ...
      <Container fluid>
        <Row>
          <Col className=" h2 pb-3">Purchase Billing Page</Col>
        </Row>
        <Row>
          <Col>PAN</Col>
          <Col>Name</Col>
              ...
              <>
                <Col md={4}>
                  ...
                  >
                    ...
                    <option>Abiraterone</option>
                    <option>Bacitracin topical</option>
